...
def read_sn_from_file() :
	...
	print "Oops! File doesn't exist", read_sn_from_file.__name__
	sys.exit()
...
def load_data_from_file( device, category ) :
	...
	try:
	    ...
	    elif( category is 'D' and onedf['Value'].min() == 0  ) : #Dust error will report 0
		print "D--NG--"
		devlist_ng.append( devname )
	    elif( category is 'L' and onedf['Value'].std() < 40 ) : # Light sensor doesn't have error value, we need to verify lux value variant
		print "L--NG--"
		devlist_ng.append( devname )
	    else :
		print "OK----"
	    ...
	    print "Oops! File doesn't exist, function name is:", load_data_from_file.__name__
	    break
